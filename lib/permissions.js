...
class PermissionHelper {
    ...
    hasRoles(roles, user, g) {
        ...
        const member = user instanceof discord_js_1.default.GuildMember ? user : guild.members.find('id', user.id);
        if (!member)
    ...
    async isAdmin(user, guild, globalOnly = false) {
        ...
        if (!g)
            return false;
        const member = user instanceof discord_js_1.default.GuildMember ? user : g.members.find('id', user.id);
