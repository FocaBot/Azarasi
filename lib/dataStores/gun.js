...
""""""""""""""""""""""""`));
            ...
            // Attach Gun endpoint
            .then(srv => new gun_1.default({ web: srv, file: path }))
            .catch(e => {
            if (e.code === 'EADDRINUSE')
                return new gun_1.default({ peers: [`http://127.0.0.1:${port}/gun`], file: path });
        })
            .catch(e => {
    ...
    async get(key) {
        ...
        return new Promise(resolve => {
            this.db.get(key).get('val').once((v) => resolve(JSON.parse(v || 'null')));
        });
    }
    async set(key, val) {
        await this.ensureReady();
    ...
    async subscribe(key, handler) {
        ...
        const gsub = this.db.get(key).get('val').on((data) => sub.handler(JSON.parse(data || 'null')));
        sub.off = () => gsub.off();
        return sub;
    }
}
